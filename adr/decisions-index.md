...
| 0021 | Auth Pulled Into Active Build — S1–S6 Decomposition + Dev-Token Posture | **Proposed** 2026-06-19 (extends 0011; re-confirms its sequencing trigger). Build order S1→S3→S2→S4→(S5,S6); non-breaking household-token coexistence (cutover at S3); dev-token local-only. **Accept before build.** |
