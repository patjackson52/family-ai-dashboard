...
// validation.ts) and the CLI (apps/cli/.../MediaValidation.kt) — a parser
// differential IS the vulnerability. The client applies it before any Coil load
// (defense in depth: the server already validated, but cached/old rows + future
// surfaces must not fetch a non-allowlisted host). Pure Kotlin, no platform deps.
object MediaValidation {
  ...
  fun imageUrlError(url: String?): String? {
    ...
    if (url.substring(0, schemeSep).lowercase() != "https") return "scheme must be https"
